...
const TOOLS = ["Notion", "Linear", "Jira", "Slack", "Teams", "Gmail", "Google Drive", "Outlook"];
...
const state = {
  categories: {},   // id -> "skip" | "light" | "heavy"
  categoryText: {},
  tools: {},
  toolsOther: "",
  habits: {},
...
function renderCategories() {
  ...
  CATEGORIES.forEach(cat => {
    ...
    div.innerHTML = `
      ...
      <input type="text" class="text-input category-text" data-id="${cat.id}" placeholder="Common use cases (optional)">
    `;
    el.appendChild(div);
  });
  ...
  document.querySelectorAll(".switch3 button").forEach(btn => {
    btn.addEventListener("click", () => {
      ...
      wrap.querySelectorAll("button").forEach(b => b.classList.remove("active", "heavy"));
      btn.classList.add("active");
      if (val === "heavy") btn.classList.add("heavy");
      const textInput = document.querySelector(`.category-text[data-id="${id}"]`);
...
function renderTools() {
  ...
  TOOLS.forEach(tool => {
    ...
    chip.addEventListener("click", () => {
      ...
      chip.classList.toggle("active", state.tools[tool]);
      updatePreview();
    });
    el.appendChild(chip);
  });

  document.getElementById("toolsOther").addEventListener("input", e => {
...
function renderHabits() {
  ...
  HABITS.forEach(group => {
    ...
    group.items.forEach(text => {
      ...
      state.habits[id] = { checked: false, text };
      const item = document.createElement("label");
      item.className = "habit-item";
      item.innerHTML = `<input type="checkbox" data-id="${id}"><span>${text}</span>`;
      item.querySelector("input").addEventListener("change", e => {
        state.habits[id].checked = e.target.checked;
        updatePreview();
      });
      el.appendChild(item);
    });
...
function wireExtras() {
  ...
  designToggle.addEventListener("change", () => {
    ...
    designFields.classList.toggle("show", state.design);
    updatePreview();
  });
  document.getElementById("designWords").addEventListener("input", e => {
    state.designWords = e.target.value;
    updatePreview();
  });
  document.getElementById("designTool").addEventListener("input", e => {
...
function buildMarkdown() {
  ...
  lines.push("");
  lines.push("These are standing instructions for how I work as a Product Manager. Follow them by default in our conversations.");
  lines.push("");

  const activeCats = CATEGORIES.filter(c => state.categories[c.id] !== "skip");
  if (activeCats.length) {
    lines.push("## Working style");
    lines.push("");
    activeCats.forEach(cat => {
      const depth = state.categories[cat.id];
      lines.push(`### ${cat.name} (${depth === "heavy" ? "Heavy" : "Light"})`);
      lines.push(cat[depth]);
      const extra = (state.categoryText[cat.id] || "").trim();
      if (extra) {
        lines.push(`My common use cases here: ${extra}`);
      }
      lines.push("");
    });
  }

  const activeTools = TOOLS.filter(t => state.tools[t]);
  const otherTools = state.toolsOther.trim();
  if (activeTools.length || otherTools) {
    lines.push("## Tools I use");
    lines.push("");
    const all = [...activeTools];
    if (otherTools) all.push(otherTools);
    lines.push(all.join(", ") + ".");
    lines.push("Reference these tools naturally when relevant, and assume I may have connectors for some of them.");
    lines.push("");
  }

  const activeHabits = Object.values(state.habits).filter(h => h.checked);
  if (activeHabits.length) {
    lines.push("## Daily habits");
    lines.push("");
    lines.push("When I ask, or at the start of a session, be ready to do the following:");
    lines.push("");
    activeHabits.forEach(h => lines.push(`- ${h.text}`));
    lines.push("");
    lines.push("Note: these run when I ask or when a session starts, not on a fixed clock, unless the surface I'm using supports scheduled tasks.");
    lines.push("");
  }
  ...
  if (state.design) {
    ...
    lines.push("");
    let block = "When asked to generate design or UI prompts, use these principles:";
    if (state.designWords.trim()) block += ` style: ${state.designWords.trim()}.`;
    if (state.designTool.trim()) block += ` Default to producing a prompt formatted for ${state.designTool.trim()}, not raw code, unless I ask for code directly.`;
    lines.push(block);
    lines.push("");
  }
  ...
  if (state.doc) {
    ...
    lines.push("");
    lines.push("When I say 'document this' or reference a project by name, generate structured documentation: a short overview, key decisions made, and open items, in plain markdown, without me having to specify format each time.");
    lines.push("");
  }

  if (!activeCats.length && !activeHabits.length && !state.design && !state.doc) {
    lines.push("_Nothing configured yet. Set your working style, habits, or extras on the left._");
  }
...
function updatePreview() {
  ...
  document.getElementById("previewBody").textContent = md;
  const lineCount = md.split("\n").filter(l => l.trim()).length;
  document.getElementById("lineCount").textContent = `${lineCount} lines`;

  const hasContent = Object.values(state.categories).some(v => v !== "skip") ||
    Object.values(state.habits).some(h => h.checked) || state.design || state.doc;
  const statusEl = document.getElementById("statusLight");
  statusEl.classList.toggle("live", hasContent);
  document.getElementById("statusText").textContent = hasContent ? "CONFIGURED" : "STANDBY";
}
